...
// [[Rcpp::plugins(cpp11)]]
// [[Rcpp::plugins(openmp)]]
void CreateHt(RaggedArray &x, const int L, int** Ht, const int n_cores) {
  // Ht is p by L
  ...
  #pragma omp parallel
  {
    ...
    mt19937_64 mt(seeds[id]);  
    for (int l=0; l<L; l++) {
      if( (l % n_cores) == id) {   // A simple way to share load
...
double PrevEst(const vector<int> &intersection, int** Ht, const int L, const double n_plus_1_over_n, const double recip_n_plus_1, const int n_cores) {
...
vector<double> PrevEst_inter(const set<vector<int> > &interactions,
			     int** Ht, const int L,
			     const double n_plus_1_over_n,
			     const double recip_n_plus_1,
			     const int n_cores) {
